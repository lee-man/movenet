...
class opts(object):
  def __init__(self):
    ...
    # model
    self.parser.add_argument('--arch', default='mobilenet', 
                             help='model architecture. Currently tested'
                                  'res_18 | res_101 | resdcn_18 | resdcn_101 |'
                                  'dlav0_34 | dla_34 | hourglass | mobilenet')
  ...
  def parse(self, args=''):
    ...
    if opt.head_conv == -1: # init default head_conv
      opt.head_conv = 256 if 'dla' in opt.arch or 'mobilenet' in opt.arch else 64
