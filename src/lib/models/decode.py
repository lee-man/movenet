...
def _topk_channel_with_reg_kps(scores, reg_kps, K=40):
      batch, cat, height, width = scores.size()
      print('scores size: ', scores.size())
      print('reg_kps size: ', reg_kps.size())
    #   exit()

      weight_to_joint = torch.zeros(scores.size())
      for ind_joint in range(17):
        for ind_y in range(height):
            for ind_x in range(width):
                distance = torch.sqrt(torch.tensor((ind_y - reg_kps[0][ind_joint][0][0][0]) * (ind_y - reg_kps[0][ind_joint][0][0][0]) + (ind_x - reg_kps[0][ind_joint][0][0][1]) * (ind_x - reg_kps[0][ind_joint][0][0][1]), dtype=torch.float64))
                weight_to_joint[0][ind_joint][ind_y][ind_x]  = 1 / (distance + 1)
    #   print(weight_to_joint.size())
    #   print(weight_to_joint)
    #   exit()
      scores *= weight_to_joint
      topk_scores, topk_inds = torch.topk(scores.view(batch, cat, -1), K)
...
def _topk_with_center(scores, K=40):
    batch, cat, height, width = scores.size()
    print('scores size: ', scores.size())

    # distance_to_center = torch.ones_like(scores)
    # center = torch.tensor((height / 2, width / 2)).reshape(1, 1, 1, 1)
    weight_to_center = torch.zeros((height, width))
    # naive implementation
    for ind_y, _ in enumerate(weight_to_center):
        for ind_x, _ in enumerate(weight_to_center):
            distance = torch.sqrt(torch.tensor((ind_y - height / 2) * (ind_y - height / 2) + (ind_x - width / 2) * (ind_x - width / 2), dtype=torch.float64))
            weight_to_center[ind_y][ind_x] = 1 / (distance + 1)
